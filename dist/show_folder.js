 ...
 */
const showFolderMain = () => {
    const getFolderIdFromArgs = () => {
    ...
    const getFolderPath = (folder, app) => {
        const path = [];
        let current = folder;
        while (current) {
    ...
    const countDirectProjects = (folder) => {
        try {
    ...
    const getFolderDetails = (folder, app) => {
        const id = folder.id();
